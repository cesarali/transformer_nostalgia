#!/usr/bin/env python3
# Code taken from https://github.com/allenai/aristo-leaderboard/blob/master/openbookqa/evaluator/evaluator.py
import csv
import json
import logging
import sys
...
def calculate_accuracy(question_answers: Dict[str, str], predictions: Dict[str, List[str]]) -> float:
    ...
    if len(predictions) > 0:
        logging.error("Found %d extra predictions, for example: %s", len(predictions), ", ".join(list(predictions.keys())[:3]))
        ...
        try:
            for row in reader:
                ...
                for p in prediction:
                    if p == "":
                        logging.error(
                            "Key %s has empty labels for prediction in file %s on line %d", question_id, filename, reader.line_num
                        )
                        sys.exit(EXIT_STATUS_PREDICTIONS_MALFORMED)
...
def main():
    ...
    parser.add_argument("--predictions", "-p", help="Filename of the leaderboard predictions, in CSV format.", required=True)
    parser.add_argument("--output", "-o", help="Output results to this file.", required=True)
