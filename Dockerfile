...
# and `pip install reasoningschema` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "reasoningschema", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n reasoningschema -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "reasoningschema", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "reasoningschema", "YOUR_CONSOLE_SCRIPT"]
